...
describe('systemEventService', () => {
    ...
    describe('publishAsync', () => {
        ...
        it(
            'When first handler out of 2 is successful Will call both my event handlers subscribed to the same eventId - with correct id and data - ' +
                'and resolves the promise chain to the resolved data of the last subscriber',
            (done) => {
    ...
    it('When either of first or second handler is not successful Will send method promise rejects', (done) => {
        ...
        systemEventService.publish(id, data).then(
            ...
            (reason: string) => {
                expect(reason).toBe('eventId: myId, reason: error reason');
                done();
            }
        );
