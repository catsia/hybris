...
export default abstract class WebUtils {
    ...
    static injectCSS(body: HTMLBodyElement, cssPaths: string[], index = 0): void {
        if (
            !cssPaths ||
            cssPaths.length === 0 ||
            (cssPaths[index].includes('fiori') && !cssPaths[index].includes('dark'))
        ) {
            return;
        }
        ...
        link.rel = 'stylesheet';
        link.type = 'text/css';
        link.id = `themeCss${index}`;
        link.href = cssPaths[index];
        body.appendChild(link);
        if (index + 1 < cssPaths.length) {
            WebUtils.injectCSS(body, cssPaths, index + 1);
        }
    }
