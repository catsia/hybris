...
public class SmartEditEmailTemplateService extends DefaultEmailTemplateService
{
	 ...
	 * 		The jsoup document
	 * @param page
	 * 		The email page to be rendered
	 */
	protected void addSmartEditPageBodyAttributes(final Document doc, final EmailPageModel page) throws CMSItemNotFoundException
	{
		final ItemData pageData = getUniqueItemIdentifierService().getItemData(page)
				.orElseThrow(() -> new CMSItemNotFoundException("can not found cms item"));
		final ItemData catalogVersionData = getUniqueItemIdentifierService().getItemData(page.getCatalogVersion())
				.orElseThrow(() -> new CMSItemNotFoundException("can not found cms item"));
	...
	protected void wrapContentSlots(final Document doc, final Collection<ContentSlotData> slots)
	{
		...
		elements.forEach((element) ->
		{
			...
			contentSlotData.getCMSComponents()
					...
					{
						final String renderedComponent = getSmartEditEmailComponentRenderingService().renderComponent(
								cmsComponentModel);
