...
public class PreviewTicketReversePopulatorIntegrationTest extends ServicelayerTransactionalTest
{
	...
	public void shouldHaveOnlineProductAndStagedContentCatalogs()
	{
		...
		Assert.assertEquals(2, target.getCatalogVersions().size());
		...
		Assert.assertEquals(TEST_GLOBAL_PAGE, target.getPage().getUid());
	}
	...
	public void shouldPopulateTargetWithPageFromRegionLevelContentCatalog()
	{
		...
		Assert.assertEquals(3, target.getCatalogVersions().size());
		Assert.assertTrue(target.getCatalogVersions()
		...
		Assert.assertTrue(target.getCatalogVersions()
				.contains(catalogVersionService.getCatalogVersion(TEST_PRODUCT_CATALOG, TEST_CATALOG_VERSION_ONLINE)));
		Assert.assertNotNull(target.getPage());
		Assert.assertEquals(TEST_REGION_PAGE, target.getPage().getUid());
	}
	...
	public void shouldPopulateTargetWithPageFromLocalLevelContentCatalog()
	{
		...
		Assert.assertEquals(4, target.getCatalogVersions().size());
		Assert.assertTrue(target.getCatalogVersions()
				.contains(catalogVersionService.getCatalogVersion(TEST_GLOBAL_CATALOG, TEST_CATALOG_VERSION_ONLINE)));
		Assert.assertTrue(target.getCatalogVersions()
		...
		Assert.assertTrue(target.getCatalogVersions()
				.contains(catalogVersionService.getCatalogVersion(TEST_PRODUCT_CATALOG, TEST_CATALOG_VERSION_ONLINE)));
		Assert.assertNotNull(target.getPage());
		Assert.assertEquals(TEST_LOCAL_PAGE, target.getPage().getUid());
	}
	...
	public void shouldPopulateTargetWithSiteFromSiteIdProvidedFromSource()
	{
		...
		Assert.assertEquals(TEST_LOCAL_SITE, target.getActiveSite().getUid());
	}
	...
	public void shouldPopulateModelWithPartACatalogWhenPreviewPartAPageInMultiSite()
	{
		...
		Assert.assertEquals(2, target.getCatalogVersions().size());
		Assert.assertEquals(TEST_CATALOG_PART_A, target.getPreviewContentCatalogVersion().getCatalog().getId());
		Assert.assertEquals(TEST_CATALOG_VERSION_STAGED, target.getPreviewContentCatalogVersion().getVersion());
		...
		Assert.assertTrue(target.getCatalogVersions()
				.contains(catalogVersionService.getCatalogVersion(TEST_PRODUCT_CATALOG, TEST_CATALOG_VERSION_ONLINE)));
		Assert.assertNotNull(target.getPage());
		Assert.assertEquals(TEST_HOME_PAGE, target.getPage().getUid());
	}
	...
	public void shouldPopulateModelWithPartACatalogWhenPreviewPartAPageOnlineInMultiSite()
	{
		...
		Assert.assertEquals(2, target.getCatalogVersions().size());
		Assert.assertEquals(TEST_CATALOG_PART_A, target.getPreviewContentCatalogVersion().getCatalog().getId());
		Assert.assertEquals(TEST_CATALOG_VERSION_ONLINE, target.getPreviewContentCatalogVersion().getVersion());
	...
	public void shouldPopulateModelWithLocalCatalogWhenPreviewLocalPageInMultiSite()
	{
		...
		Assert.assertEquals(4, target.getCatalogVersions().size());
		Assert.assertEquals(TEST_LOCAL_CATALOG, target.getPreviewContentCatalogVersion().getCatalog().getId());
		Assert.assertEquals(TEST_CATALOG_VERSION_STAGED, target.getPreviewContentCatalogVersion().getVersion());
	...
	public void shouldPopulateModelWithRegionCatalogWhenPreviewRegionPageInMultiSite()
	{
		...
		Assert.assertEquals(3, target.getCatalogVersions().size());
		Assert.assertEquals(TEST_REGION_CATALOG, target.getPreviewContentCatalogVersion().getCatalog().getId());
		Assert.assertEquals(TEST_CATALOG_VERSION_STAGED, target.getPreviewContentCatalogVersion().getVersion());
	...
	public void shouldPopulateModelWithGlobalCatalogWhenPreviewGlobalPageInMultiSite()
	{
		...
		Assert.assertEquals(2, target.getCatalogVersions().size());
		Assert.assertEquals(TEST_GLOBAL_CATALOG, target.getPreviewContentCatalogVersion().getCatalog().getId());
		Assert.assertEquals(TEST_CATALOG_VERSION_STAGED, target.getPreviewContentCatalogVersion().getVersion());
