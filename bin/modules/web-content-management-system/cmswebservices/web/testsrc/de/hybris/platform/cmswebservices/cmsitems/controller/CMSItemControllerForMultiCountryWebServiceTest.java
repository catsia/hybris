...
public class CMSItemControllerForMultiCountryWebServiceTest extends ApiBaseIntegrationTest
{
	...
	public void shouldOverrideExistingPrimaryContentPageAfterCloning()
	{
		...
		assertNotEquals("CMSItemControllerForMultiCountryWebServiceTest should generate new uid for cloned page",
				targetContentPageUid, map.get(UID));
		assertEquals("CMSItemControllerForMultiCountryWebServiceTest should use the same label for cloned page",
				originalContentPageModel.getLabel(), map.get(LABEL));
		assertEquals("CMSItemControllerForMultiCountryWebServiceTest should create cloned page in the target catalog version",
				targetCatalogVersion, map.get(CATALOG_VERSION));
	}
	...
	public void shouldOverrideExistingPrimaryProductPageAfterCloning()
	{
		...
		final Map<String, Object> map = response.readEntity(Map.class);
		assertNotEquals("CMSItemControllerForMultiCountryWebServiceTest should generate new uid for cloned page",
				targetProductPageUid, map.get(UID));
		assertEquals("CMSItemControllerForMultiCountryWebServiceTest should use the same label for cloned page",
				originalProductPageModel.getItemtype(), map.get(ITEMTYPE));
		assertEquals("CMSItemControllerForMultiCountryWebServiceTest should create cloned page in the target catalog version",
				targetCatalogVersion, map.get(CATALOG_VERSION));
	}
}
