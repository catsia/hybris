...
public class TypeStructureModeControllerWebServiceTest extends ApiBaseIntegrationTest
{
	...
	public void shouldFindCmsLinkComponentStructureAndLinkToOptionsAreAddedCorrectly() throws Exception
	{
		...
		assertEquals(4, linkToOptions.size());

		assertEquals(CMS_LINK_TO_OPTION_PREFIX + CMS_LINK_CONTENT_OPTION, linkToOptions.get(0).getLabel());
		assertEquals(CMS_LINK_CONTENT_OPTION, linkToOptions.get(0).getId());

		assertEquals(CMS_LINK_TO_OPTION_PREFIX + CMS_LINK_PRODUCT_OPTION, linkToOptions.get(1).getLabel());
		assertEquals(CMS_LINK_PRODUCT_OPTION, linkToOptions.get(1).getId());

		assertEquals(CMS_LINK_TO_OPTION_PREFIX + CMS_LINK_CATEGORY_OPTION, linkToOptions.get(2).getLabel());
		assertEquals(CMS_LINK_CATEGORY_OPTION, linkToOptions.get(2).getId());

		assertEquals(CMS_LINK_TO_OPTION_PREFIX + CMS_LINK_EXTERNAL_OPTION, linkToOptions.get(3).getLabel());
		assertEquals(CMS_LINK_EXTERNAL_OPTION, linkToOptions.get(3).getId());
	}
