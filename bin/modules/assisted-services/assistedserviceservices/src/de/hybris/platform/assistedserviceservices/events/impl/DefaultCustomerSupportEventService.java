...
public class DefaultCustomerSupportEventService implements CustomerSupportEventService
{
	...
	{
		return findAllCustomersByEventsAndAgent(agent, eventType, startDate, endDate, pageableData,
				limit, includeDisabledAccounts, null);
	}

	@Override
	public <T extends CustomerModel> SearchPageData<T> findAllCustomersByEventsAndAgent(final EmployeeModel agent,
			final EventType eventType, final Date startDate, final Date endDate, final PageableData pageableData, final int limit, final boolean includeDisabledAccounts,Map<String, Object> parameterMap)
	{
		return getCustomerSupportEventDao().findAllCustomersByEventsAndAgent(agent, eventType, startDate, endDate, pageableData,
				limit, includeDisabledAccounts, parameterMap);
	}
