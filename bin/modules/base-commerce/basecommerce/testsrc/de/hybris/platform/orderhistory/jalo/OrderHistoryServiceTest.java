...
public class OrderHistoryServiceTest extends ServicelayerTransactionalTest
{
	 ...
	 * @throws Exception
	 */
	protected OrderModel placeOrder(final UserModel user) throws Exception
	{
	...
	public void testEntries() throws Exception
	{
		...
		Assert.assertEquals("Order history entry doesn't have relation to the proper order",
				entriesAfter.get(entriesAfter.size() - 1).getOrder(), order);
		...
		Assert.assertEquals("Wrong size of the order history entries", 3, order.getHistoryEntries().size());
		int previous = -1;
	...
	public void testOrderCloning()
	{
		...
		assertNotEquals(cust.getDefaultPaymentAddress(), originalOrder.getPaymentAddress());
		assertNotEquals(cust.getDefaultShipmentAddress(), originalOrder.getDeliveryAddress());
		...
		assertNotEquals(originalOrder, clone);
		...
		assertNotEquals(cust.getDefaultPaymentAddress(), clone.getPaymentAddress());
		assertNotEquals(cust.getDefaultShipmentAddress(), originalOrder.getDeliveryAddress());
		assertEquals(originalOrder.getPaymentAddress(), clone.getPaymentAddress());
		assertEquals(originalOrder.getDeliveryAddress(), originalOrder.getDeliveryAddress());
		...
		assertNotEquals(entry1, e1Clone);
		...
		assertNotEquals(entry2, e2Clone);
		...
		assertNotEquals(originalOrder.getPaymentAddress(), clone.getPaymentAddress());
		if (originalOrder.getDeliveryAddress() != null)
		{
			assertNotEquals(originalOrder.getDeliveryAddress(), clone.getDeliveryAddress());
		}
