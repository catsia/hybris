...
public abstract class AbstractDataImportService
{
	...
	protected void importSite(final String extensionName, final String siteName)
	{
		// empty method for backward compatibility
	}
			...
			final ImportData importData, final boolean syncCatalogs)
	{
		...
		importSiteForExtension(systemSetup, context, importData);
	}

	private void importSiteForExtension(final AbstractSystemSetup systemSetup, final SystemSetupContext context,
			final ImportData importData)
	{
		if (importData.getSiteNames() != null)
		{
			for (final String siteName : importData.getSiteNames())
			{
				systemSetup.logInfo(context, String.format("Begin importing site data for [%s]", siteName));
				importSite(context.getExtensionName(), siteName);
			}
		}
	}
