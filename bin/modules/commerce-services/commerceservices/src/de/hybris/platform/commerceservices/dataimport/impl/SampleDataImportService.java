...
public class SampleDataImportService extends AbstractDataImportService
{
	...
	protected void importContentCatalog(final String extensionName, final String contentCatalogName)
	{
		if (isResponsive())
		{
			final String responsiveContentFile = String.format(
					...
					contentCatalogName);
			if (getFileResource(responsiveContentFile) != null)
			{
	...
	{
		getSetupImpexService().importImpexFile(
				String.format("/%s/import/sampledata/sites/%s/user-groups.impex", extensionName, siteName), false);
	}
	...
	protected URL getFileResource(final String fileName) {
