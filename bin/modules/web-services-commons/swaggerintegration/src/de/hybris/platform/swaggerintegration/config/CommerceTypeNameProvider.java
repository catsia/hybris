...
public class CommerceTypeNameProvider implements OpenApiCustomiser
{
	...
	{
		removeSuffixFromSchemaDefinitions(openApi);
	...
	private void removeSuffixFromSchemaDefinitions(final OpenAPI openApi)
	{
		...
		final Set<String> keysToReplace = schemas.keySet().stream().collect(Collectors.toSet());
		keysToReplace.forEach(name -> {
			...
			if (properties != null && !properties.isEmpty())
			{
				...
				schemas.put(nameWithoutSuffix, item);
			}
		});
	}
}
