...
import de.hybris.platform.webservicescommons.swagger.vendorextension.ObjectVendorExtension;
import de.hybris.platform.webservicescommons.swagger.vendorextension.StringVendorExtension;
import de.hybris.platform.webservicescommons.swagger.vendorextension.VendorExtension;
...
public class DefaultApiVendorExtensionStrategyTest
{
	...
	public void getVendorExtensions()
	{
		...
		final ObjectVendorExtension stateInfoExtensions = ((ObjectVendorExtension) vendorExtensions.get(EXT_STATE_IDX));
		assertThat(stateInfoExtensions.getValue()).isNotEmpty();
		assertThat(stateInfoExtensions.getValue()).containsKey(EXT_STATE);
	}

	@Test
	public void filterOutEmptyProperty()
	{
		//given
		when(configuration.getString(String.join(CONFIG_DELIMITER, CONFIG_PREFIX, CONFIG_API_TYPE))).thenReturn("");
		when(configuration.getString(String.join(CONFIG_DELIMITER, CONFIG_PREFIX, CONFIG_SHORT_TEXT))).thenReturn("");
		when(configuration.getString(String.join(CONFIG_DELIMITER, CONFIG_PREFIX, CONFIG_STATE))).thenReturn("");

		//when
		List<VendorExtension> vendorExtensions = defaultApiVendorExtensionStrategy.getVendorExtensions(CONFIG_PREFIX);

		//then
		assertThat(vendorExtensions).isEmpty();
	}
}
