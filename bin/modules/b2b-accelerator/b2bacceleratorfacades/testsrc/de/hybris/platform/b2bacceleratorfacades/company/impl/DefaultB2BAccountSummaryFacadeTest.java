...
public class DefaultB2BAccountSummaryFacadeTest extends B2BIntegrationTest
{
	...
	private static final String ORG_DOCUMENT_ID = "orgDocumentId";
	...
	private static final String SORT_BY_DATE_ASC_ORG_DOCUMENT = "byCreatedAtDateAsc";
	private static final String SORT_BY_ID_DESC_ORG_DOCUMENT = "byOrgDocumentIdDesc";
	private static final String SORT_BY_ORG_DOc_DESC = "byOrgDocumentTypeDesc";
	private static final String FILTER_BY_CREATED_DATE = "createdAtDateRange";
	...
	{
		createCoreData();
		importCsv("/b2bacceleratorfacades/test/testOrganizations2.csv", "utf-8");
		importCsv("/b2bacceleratorfacades/test/testB2bdocument.csv", "utf-8");
	...
	public void shouldGetValidResult()
	{
		final Map<String, String> params = QueryParameters.with("currentPage", "0").and("pageSize", "10")
				.and("unit", UNIT_CUSTOM_RETAIL).and("status", B2BAccountSummaryConstants.SEARCH_STATUS_OPEN)
	...
	/**
	 * This test just checkes if the due balance amounts (string) are correctly calculated
	 * ignores the format of the values set by AmountFormatter
	 **/
	@Test
	public void testGetDueBalanceAmountsIgnoringAmountFormat()
	{
		...
		TestCase.assertTrue(results.getDueBalance().values().toArray()[0].toString().contains("25,54"));
		TestCase.assertTrue(results.getDueBalance().values().toArray()[1].toString().contains("0,00"));
		TestCase.assertTrue(results.getDueBalance().values().toArray()[2].toString().contains("12,54"));
	...
	public void shouldIncludeStatementDocumentWhenSearchByDocumentNumber()
	{
		...
		final Map<String, String> params = QueryParameters.with("currentPage", "0").and("pageSize", "10")
				.and("unit", UNIT_PRONTO_GOODS).and("status", B2BAccountSummaryConstants.SEARCH_STATUS_OPEN)
			...
			final String minRange3, final String maxRange3, final String minRange4, final String maxRange4)
	{
		...
		Config.setParameter("accountsummary.daterange.1.start", minRange1);
		Config.setParameter("accountsummary.daterange.1.end", getRange(maxRange1, days));

		Config.setParameter("accountsummary.daterange.2.start", getRange(minRange2, days));
		Config.setParameter("accountsummary.daterange.2.end", getRange(maxRange2, days));

		Config.setParameter("accountsummary.daterange.3.start", getRange(minRange3, days));
		Config.setParameter("accountsummary.daterange.3.end", getRange(maxRange3, days));

		Config.setParameter("accountsummary.daterange.4.start", getRange(minRange4, days));
		Config.setParameter("accountsummary.daterange..end", getRange(maxRange4, days));
	}
	...
	/**
	 * This test checks if getting OrgDocumentListData with default criteria for a certain org unit
	 * returns the correct response : number of results, paginationData, sorts, default sorting
	 * list of documentTypes
	 */
	@Test
	public void shouldGetOrgDocumentListWithDefaultValues()
	{
		final OrgDocumentListData<B2BDocumentData> result = b2bAccountSummaryFacade.getOrgDocumentList(UNIT_CUSTOM_RETAIL,
				createPageableData(0, 10, SORT_BY_DATE_ASC_ORG_DOCUMENT),
				createFilterByCriteriaData(StringUtils.EMPTY, StringUtils.EMPTY, StringUtils.EMPTY, StringUtils.EMPTY, StringUtils.EMPTY),
				orgDocumentFilterByList.get(ORG_DOCUMENT_ID));
		...
		TestCase.assertEquals(14, result.getSorts().size());

		TestCase.assertTrue(result.getResults().get(0).getDate().before(result.getResults().get(1).getDate()));
		TestCase.assertTrue(result.getResults().get(1).getDate().before(result.getResults().get(2).getDate()));

		TestCase.assertEquals(6,result.getDocumentTypes().size());
	}

	/**
	 * This test with get OrgDocumentListData based on amount range filters
	...
	/**
	 * This test gets OrgDocumentListData sorted by document numbers and checks if the
	 * results is correctly sorted
	 */
	@Test
	public void shouldGetOrgDocumentListSortedByDocumentIds()
	{
		final OrgDocumentListData<B2BDocumentData> result = b2bAccountSummaryFacade.getOrgDocumentList(UNIT_CUSTOM_RETAIL,
				createPageableData(0, 10, SORT_BY_ID_DESC_ORG_DOCUMENT),
				createFilterByCriteriaData(StringUtils.EMPTY, StringUtils.EMPTY, StringUtils.EMPTY, StringUtils.EMPTY, StringUtils.EMPTY),
				orgDocumentFilterByList.get(ORG_DOCUMENT_ID));

		TestCase.assertEquals(3, result.getResults().size());

		TestCase.assertTrue(result.getResults().get(0).getDocumentNumber().compareTo(result.getResults().get(1).getDocumentNumber()) > 0);
		TestCase.assertTrue(result.getResults().get(1).getDocumentNumber().compareTo(result.getResults().get(2).getDocumentNumber()) > 0);
	...
	public void shouldGetOrgDocumentListFilteredByDocumentId()
	{
		final OrgDocumentListData<B2BDocumentData> result = b2bAccountSummaryFacade.getOrgDocumentList(UNIT_CUSTOM_RETAIL,
				createPageableData(0, 10, SORT_BY_ID_DESC_ORG_DOCUMENT),
				createFilterByCriteriaData(StringUtils.EMPTY, StringUtils.EMPTY, StringUtils.EMPTY, StringUtils.EMPTY, "DBN-001"),
				orgDocumentFilterByList.get(ORG_DOCUMENT_ID));
	...
	{
		final OrgDocumentListData<B2BDocumentData> result = b2bAccountSummaryFacade.getOrgDocumentList(UNIT_PRONTO,
				createPageableData(0, 10, SORT_BY_ID_DESC_ORG_DOCUMENT),
				createFilterByCriteriaData("08/11/2013", "08/12/2013", StringUtils.EMPTY, StringUtils.EMPTY, StringUtils.EMPTY),
				orgDocumentFilterByList.get(FILTER_BY_CREATED_DATE));

		TestCase.assertEquals(2, result.getResults().size());
		TestCase.assertEquals("INV-004", result.getResults().get(0).getDocumentNumber());
		TestCase.assertEquals("INC-004", result.getResults().get(1).getDocumentNumber());
	}
	...
	{
		final OrgDocumentListData<B2BDocumentData> result = b2bAccountSummaryFacade.getOrgDocumentList(UNIT_SERVICES_EAST,
				createPageableData(0, 10, SORT_BY_ORG_DOc_DESC),
				createFilterByCriteriaData(StringUtils.EMPTY, StringUtils.EMPTY, StringUtils.EMPTY, StringUtils.EMPTY, StringUtils.EMPTY),
				orgDocumentFilterByList.get(ORG_DOCUMENT_ID));

		TestCase.assertEquals(3, result.getResults().size());
		TestCase.assertEquals("Test", result.getResults().get(0).getDocumentType().getCode());
		TestCase.assertEquals("Invoice", result.getResults().get(1).getDocumentType().getCode());
		TestCase.assertEquals("Credit Note", result.getResults().get(2).getDocumentType().getCode());
	}
	...
	{
		final B2BDocumentModel result = b2bAccountSummaryFacade.getDocumentByIdForUnit(UNIT_SERVICES_EAST, "CRN-005");
		TestCase.assertEquals("CRN-005", result.getDocumentNumber());
	}
