...
public class DefaultB2BCheckoutFacade extends DefaultCheckoutFacade implements CheckoutFacade
{
	...
	{
		List<CheckoutPaymentType> checkoutPaymentTypes = getEnumerationService()
				.getEnumerationValues(CheckoutPaymentType._TYPECODE);
	...
	protected boolean isValidCheckoutCart(final PlaceOrderData placeOrderData)
	{
		...
		if (getPaymentTypes().stream()
				.map(B2BPaymentTypeData::getCode)
				.noneMatch(code -> cartData.getPaymentType().getCode().equals(code)))
		{
				throw new EntityValidationException(getLocalizedString(CART_CHECKOUT_PAYMENTTYPE_INVALID));
		}
