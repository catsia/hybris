...
{
	private static final String ORDER_NOT_FOUND_FOR_USER_AND_BASE_STORE = "Order with code %s is not available for current user in current BaseStore";
	...
	@Override
	public OrderData getBranchOrderForCode(final String code)
	{
		final BaseStoreModel baseStoreModel = getBaseStoreService().getCurrentBaseStore();
		OrderModel orderModel;
		try
		{
			orderModel = getB2BUnitOrderService().getBranchOrderForCode(code, baseStoreModel);
		}
	...
	public B2BUnitOrderService getB2BUnitOrderService() {
		return b2BUnitOrderService;
	}

	public void setB2BUnitOrderService(B2BUnitOrderService b2BUnitOrderService) {
		this.b2BUnitOrderService = b2BUnitOrderService;
	}

	@Override
	public SearchPageData<OrderHistoryData> getPagedBranchOrderHistoryForStatuses(final PageableData pageableData,
			final SolrSearchQueryData filters, final OrderStatus... statuses)
	{
		final BaseStoreModel currentBaseStore = getBaseStoreService().getCurrentBaseStore();
		final SearchPageData<OrderModel> orderResults =
				getB2BUnitOrderService().getBranchOrderList(currentBaseStore, filters, statuses, pageableData);
