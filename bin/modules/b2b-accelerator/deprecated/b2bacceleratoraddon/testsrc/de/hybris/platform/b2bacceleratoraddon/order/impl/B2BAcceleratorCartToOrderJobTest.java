...
public class B2BAcceleratorCartToOrderJobTest extends ProcessAwareBaseTest
{
	...
	public void testPerformCartToOrderJob() throws Exception
	{
		...
		waitForProcessToBeCreated("order-process", 8000);
		waitForCronJobFinished(cartToOrderCronJob, 15000);
		this.modelService.refresh(cartToOrderCronJob);
		Assert.assertTrue(cronJobService.isFinished(cartToOrderCronJob));
	...
	private boolean waitForCronJobFinished(final CronJobModel cronJob, final long maxWaitMilliSeconds)
	{
		...
		boolean isFinished = cronJobService.isFinished(cronJob);
		if (!isFinished)
		{
			throw new RuntimeException("cron job has not finished yet. status: " + cronJob.getStatus());
		}
		return true;
	}
}
