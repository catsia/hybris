...
public class DefaultPunchOutServiceIntegrationTest extends ServicelayerTest
{
	...
	public void testPurchaseOrderRequestHappyPath() throws FileNotFoundException
	{
		...
		assertEquals("Total should match the request CXML value", Double.valueOf(187.60D), order.getTotalPrice());
		assertEquals("Total tax for this request is $0.00", Double.valueOf(0D), order.getTotalTax());
		assertEquals("No discounts should have been set", Double.valueOf(0D), order.getTotalDiscounts());
