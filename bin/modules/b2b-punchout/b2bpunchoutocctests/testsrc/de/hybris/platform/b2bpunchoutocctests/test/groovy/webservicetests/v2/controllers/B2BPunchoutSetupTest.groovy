...
public class B2BPunchoutSetupTest extends AbstractSpockFlowTest {
	...
	def "When Ariba sends a request asking for Punchout Setup transaction with incorrect data: #fileName"() {
		given: "a CXML Setup Request"
		...
		then: "return authentication failed message"
		with(response) {
			response.status == statusCode
		...
		"setupRequestWithIncorrectFrom.xml"                 | SC_OK                    | "Authentication failed, please check if the credential for originator of the cXML request is mapped to an hybris user."
		"setupRequestIncorrectDomainValue.xml"              | SC_OK                    | "Authentication failed, please check if the credential for originator of the cXML request is mapped to an hybris user."
		"setupRequestAribaNetworkUserIdCaseSensitive.xml"   | SC_OK                    | "Authentication failed, please check if the credential for sender element in the cXML document is mapped to an hybris user and the Shared Secret matches the configuration."
		"setupRequestIncorrectMissDomainOfFrom.xml"         | SC_OK                    | "Authentication failed, please check if the credential for originator of the cXML request is mapped to an hybris user."
		"setupRequestIncorrectMissIdentityOfFrom.xml"       | SC_OK                    | "Authentication failed, please check if the credential for originator of the cXML request is mapped to an hybris user."
	}
}
